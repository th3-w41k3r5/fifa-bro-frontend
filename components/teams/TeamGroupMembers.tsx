...
interface TeamGroupMembersProps {
  teams: TeamSummary[];
  excludeTeam?: string;
}

export default function TeamGroupMembers({ teams, excludeTeam }: TeamGroupMembersProps) {
  const otherTeams = excludeTeam ? teams.filter((team) => team.name !== excludeTeam) : teams;

  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
      {otherTeams.map((team) => (
        <TeamMemberCard key={team.id || team.code || team.name} team={team} />
      ))}
    </div>
  );
}

function TeamMemberCard({ team }: { team: TeamSummary }) {
  return (
        ...
        <div className="relative z-10 flex items-center gap-4">
          {team.flagCode && (
            <div className="shrink-0 overflow-hidden rounded-lg border border-white/[0.08] shadow-[0_12px_30px_rgba(0,0,0,0.25)]">
              <Flag flagCode={team.flagCode} size="md" variant="square" />
            </div>
          )}

          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-accent">
              {team.groupCode ? `Group ${team.groupCode}` : 'Member'}
            </p>
            <h3 className="mt-1 truncate text-lg font-black text-text-primary">{team.name}</h3>
